...
int main(int argc, char *argv[])
{
	...
	int a, num;
	FILE *fptr;
	printf("To start transmitting weights press 0");
	scanf("%d", &a);
	if (a == 0)
	{
		fptr = fopen("/weights/weights.txt", "r");
		...
		printf("Weights transmiited succesfully");
	}

	printf("To start transmitting image press 0");
	scanf("%d", &a);

	if (a == 0)
	{
		fptr = fopen("/Input/image.txt", "r");
		while ((num = getw(fptr)) != EOF)
		{
			sendbuf[tx_len++] = num;
			// send packet
		...
		printf("Image transmiited succesfully");
	}

	fptr = fopen("/Output/image.txt", "w");
	...
	while (recvbuf[0] != 'n')
	{
		...
		// Correct packet reception
			...
			fwrite(recvbuf, sizeof(char), sizeof(recvbuf), fptr);
	}
